...
### 背景
現在、ブログ記事詳細ページでは`prose`クラスが適用されているものの、
見出しと本文の区別が視覚的に分かりにくい状況にある。
...
### 新規作成ファイル
- `presentation/utils/prose-styles.ts` - スタイル生成関数集（純粋関数）

### 更新ファイル
- `infrastructure/utils/html-to-react.ts` - スタイル適用処理を追加
- `presentation/utils/index.ts` - スタイル生成関数をエクスポート
...
| `html-to-react.ts` | `prose-styles.ts` | インポート（関数呼び出し） |
